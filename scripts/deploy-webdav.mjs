...
const localDistDir = path.resolve(
  projectRoot,
  process.env.WEBDAV_LOCAL_DIR || "dist",
);
...
async function main() {
  const skipBuild =
    (process.env.WEBDAV_SKIP_BUILD || "").toLowerCase() === "true";
  ...
  const cleanRemote =
    (process.env.WEBDAV_CLEAN_REMOTE || "").toLowerCase() === "true";
  ...
  if (!skipBuild) {
    ...
    if (buildResult.status !== 0) {
      throw new Error(
        `Build failed; deployment aborted. Exit code: ${buildResult.status ?? "unknown"}`,
      );
    }
  }
  ...
  if (!distStats || !distStats.isDirectory()) {
    throw new Error(
      `Local build directory not found: ${localDistDir}. Run "npm run build" first.`,
    );
  }
  ...
  console.log(
    `Uploading ${files.length} files from ${localDistDir} to ${remoteBaseDir} ...`,
  );

  for (const filePath of files) {
    const relativePath = path
      .relative(localDistDir, filePath)
      .split(path.sep)
      .join("/");
    const remoteFilePath = normalizeRemotePath(
      path.posix.join(remoteBaseDir, relativePath),
    );
    await ensureDirectory(
      client,
      normalizeRemotePath(path.posix.dirname(remoteFilePath)),
    );
    await client.putFileContents(remoteFilePath, await readFile(filePath), {
      overwrite: true,
    });
